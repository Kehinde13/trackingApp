...
export function TrackingView() {
  ...
  return <main className="public-track-page">
    ...
    <section className="public-timeline"><h2>Tracking timeline</h2><p>Updates may come from the carrier, shipping team, or ParcelTrack system.</p>{shipment.events.length ? <ol>{shipment.events.map((event, index) => <li key={`${event.occurredAt ?? event.providerOccurredAt}-${index}`}><span className="timeline-dot" aria-hidden="true" /><div className="public-timeline-event"><div className="timeline-event-heading"><strong>{event.description}</strong><span className="source-label">{event.sourceLabel}</span></div><p>{getStatusPresentation(event.status).label}{event.location ? ` · ${event.location}` : event.city || event.countryCode ? ` · ${showPlace(event.city, event.countryCode)}` : ""}</p>{event.occurredAt ? <time dateTime={event.occurredAt}>{dateTime(event.occurredAt)}</time> : <span>{event.providerOccurredAt} (carrier local time)</span>}</div></li>)}</ol> : <p>No tracking events are available yet.</p>}</section>
